                        ...
                        <%-- 1. Status: DRAFT / PENDING_REVIEW --%>
                            <c:when test="${canRequestEdit}">
                                ...
                                <c:if test="${sessionScope.user.roleId == 2}">
                                    <button type="button" onclick="showFeedbackModal()" class="btn btn-orange">Request
                                        edit</button>
                            ...
                            <%-- 2. TRẠNG THÁI: CUSTOMER_CHECK --%>
                                <c:when test="${canCustomerCheck}">
                                    ...
                                    <c:if test="${sessionScope.user.roleId == 3}">
                                        <button type="button" onclick="showFeedbackModal()" class="btn btn-orange">Edit
                                            request</button>
                                        <form method="POST" action="contract-detail" style="margin:0;">
                            ...
                            <div style="max-height: 600px; overflow-y: auto;">
                                <c:forEach var="h" items="${historyList}">
                                    <div
                                        ...
                                        <p style="margin: 0 0 5px 0; color: #666; font-size: 0.9em;">🕒 ${h.createdAt}
                                        </p>
                                        ...
                                        <c:if test="${sessionScope.user.roleId== 5}">
                                            <p style="margin: 0 0 10px 0; color: red"><strong>Note:</strong> ${h.note}
                                            </p>
                                        </c:if>
                                        <c:if test="${not empty h.revisionItems}">
                                            <button class="btn btn-blue"
                                                style="padding: 5px 10px; font-size: 0.85em; width: auto;"
                                                onclick="viewHistoryDetail(${h.historyId})">View detail</button>
                                            <div id="history-data-${h.historyId}" style="display:none;">
                                                <c:forEach var="item" items="${h.revisionItems}">
                                                    <div
                                                        style="border-left: 3px solid #007bff; padding: 8px; margin-bottom: 5px; background: #f8f9fa;">
                                                        <strong>Location:</strong> ${item.revisionType} <br>
                                                        <strong>Change:</strong> ${item.revisionDetail}
                                                    </div>
                                                </c:forEach>
                                            </div>
                                        </c:if>
                ...
                <form method="POST" action="contract-detail" id="feedbackForm">
                    ...
                    <div id="revisionContainer">
                        <div class="revision-box">
                            <input type="text" name="revision_type[]" placeholder="Location (Ex: Name...)" required
                                style="width:40%; padding: 8px;">
                            <input type="text" name="revision_detail[]" placeholder="Change information" required
                                style="width:50%; padding: 8px;">
                        </div>
                    </div>
                    <button type="button" onclick="addRevisionBox()" class="btn btn-add">+ Add more</button>
                    <div style="margin-top: 20px; text-align: right;">
                        <button type="button" onclick="hideModals()" class="btn btn-red"
