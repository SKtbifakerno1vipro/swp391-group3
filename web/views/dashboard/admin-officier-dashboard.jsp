        ...
        <style>
            ...
            * {
            ...
            .nav-link {
                ...
                transition:transform .28s ease,color .28s ease,box-shadow .28s ease,background-color .28s ease;
            }
            .nav-link::before {
                content:'';
                position:absolute;
            ...
            <div class="main">
                ...
                <div class="content-grid">
                    ...
                    <div class="content-card">
                        <h3>Import Request (Chờ duyệt)</h3>
                        <div style="overflow-y:auto; height: 100%;">
                            <table class="data-table">
                                ...
                                <tbody>
                                    <c:forEach items="${pendingImportRequests}" var="ir">
                                        <tr>
                                            <td>#${ir.importId}</td>
                                            <td>${ir.productName}</td>
                                            <td>${ir.quantity}</td>
                                            <td>${ir.creatorName}</td>
                                            <td><fmt:formatDate value="${ir.createdDate}" pattern="MM/dd/yyyy"/></td>
                                        </tr>
                                    </c:forEach>
                                    <c:if test="${empty pendingImportRequests}">
                                        <tr><td colspan="5" style="text-align:center;">Không có request nào chờ</td></tr>
                                    </c:if>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <!-- Column 2: Báo giá chờ xử lý -->
                    <div class="content-card">
                        <h3>Báo giá chờ xử lý</h3>
                                ...
                                <tbody>
                                    <c:forEach var="q" items="${quotationsNeedingAction}">
                                        <tr>
                                            ...
                                            <td>
                                                <c:if test="${not empty q.createdAt}">
                                                    <fmt:formatNumber value="${q.createdAt.monthValue}" minIntegerDigits="2"/>/<fmt:formatNumber value="${q.createdAt.dayOfMonth}" minIntegerDigits="2"/>/${q.createdAt.year}
                                                </c:if>
                                            </td>
                                            <td>
                                    ...
                                    <c:if test="${empty quotationsNeedingAction}">
                                        <tr><td colspan="4" style="text-align:center; color:var(--muted); padding:20px;">Không có báo giá chờ.</td></tr>
                                    </c:if>
                                </tbody>
                            </table>
                            <c:if test="${quotationsNeedingAction.size() >= 5}">
                                <div style="text-align:center; margin-top: 10px;">
                                    <a href="${pageContext.request.contextPath}/quotation-list" style="color:var(--primary); font-size: 13px; text-decoration: none;">Ấn vào đây để xem thêm</a>
                                </div>
                            </c:if>
                        </div>
                    </div>

                    <!-- Column 3: Hợp đồng chờ xử lý -->
                    <div class="content-card">
                        <h3>Hợp đồng chờ xử lý</h3>
                        <div class="table-wrapper">
                            <table class="data-table">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <c:forEach var="c" items="${contractsNeedingAction}">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${c.contractStatus == 'PENDING_REVIEW'}"><c:set var="bg" value="#f9ebce" /><c:set var="col" value="var(--tertiary)" /></c:when>
                                        </c:choose>
                                        <tr>
                                            ...
                                            <td>
                                                <c:if test="${not empty c.updatedAt}">
                                                    <fmt:formatNumber value="${c.updatedAt.monthValue}" minIntegerDigits="2"/>/<fmt:formatNumber value="${c.updatedAt.dayOfMonth}" minIntegerDigits="2"/>/${c.updatedAt.year}
                                                </c:if>
                                            </td>
                                            <td>
                                    ...
                                    <c:if test="${empty contractsNeedingAction}">
                                        <tr><td colspan="5" style="text-align:center; color:var(--muted); padding:20px;">Không có hợp đồng chờ.</td></tr>
                                    </c:if>
                                </tbody>
                            </table>
                            <c:if test="${contractsNeedingAction.size() >= 5}">
                                <div style="text-align:center; margin-top: 10px;">
                                    <a href="${pageContext.request.contextPath}/contract-list" style="color:var(--primary); font-size: 13px; text-decoration: none;">Ấn vào đây để xem thêm</a>
                                </div>
                            </c:if>
                        </div>
                    </div>
                    ...
                    <div class="content-card">
                        ...
                        <div class="table-wrapper">
                            ...
                            <c:if test="${recentInvoices.size() >= 5}">
                                <div style="text-align:center; margin-top: 10px;">
                                    <a href="${pageContext.request.contextPath}/invoice-list" style="color:var(--primary); font-size: 13px; text-decoration: none;">Ấn vào đây để xem thêm</a>
                                </div>
                            </c:if>
                        </div>
                    </div>
