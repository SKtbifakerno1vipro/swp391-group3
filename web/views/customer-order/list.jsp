        ...
        <div class="dashboard-shell">
            ...
            <main class="main legacy-page">
                ...
                <form action="customer-order-list" method="GET"
                    ...
                    <div>
                        <input type="text" placeholder="Tìm kiếm theo tên hoặc mã số thuế" name="keyword"
                               value="${keyword}" style="width: 260px;">
                    </div>
                    <div>
                        ...
                        <select name="sortBy" id="sortBy">
                            ...
                            <option value="customerName" ${sortBy=='customerName' ? 'selected' : '' }>Tên khách
                                hàng</option>
                    ...
                    <tbody>
                        <c:forEach var="item" items="${orders}">
                            ...
                            <tr>
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${item.customerOrder.orderStatus == 'PENDING'}">Chờ xử lý
                                        </c:when>
                                        <c:when test="${item.customerOrder.orderStatus == 'SHIPPING'}">Đang giao
                                            hàng</c:when>
                                        <c:when test="${item.customerOrder.orderStatus == 'CANCELLED'}">Đã hủy
                                        </c:when>
                                        <c:when test="${item.customerOrder.orderStatus == 'COMPLETED'}">Đã hoàn
                                            thành</c:when>
                                        <c:otherwise>${item.customerOrder.orderStatus}</c:otherwise>
                                    </c:choose>
                                </td>
                                <td>
                                    <fmt:parseDate value="${item.customerOrder.createdAt}"
                                                   pattern="yyyy-MM-dd'T'HH:mm" var="parsedDateTime" type="both" />
                                    <fmt:formatDate value="${parsedDateTime}" pattern="dd/MM/yyyy HH:mm" />
                                </td>
                                <td>
                                    <a
                                        href="${pageContext.request.contextPath}/customer-order?id=${item.customerOrder.customerOrderId}">Xem</a>
                                    <c:if test="${sessionScope.user.roleId != 3}">
                                        <c:if test="${item.customerOrder.orderStatus != 'COMPLETED'}">
                                            <c:if
                                                test="${item.customerOrder.orderStatus != 'SHIPPING' && item.customerOrder.orderStatus != 'CANCELLED'}">
                                                <a href="${pageContext.request.contextPath}/customer-order?action=delete_order&id=${item.customerOrder.customerOrderId}"
                                                   style="color: red;"
                                            ...
                                            <c:otherwise>
                                                <c:choose>
                                                    <c:when test="${sessionScope.user.roleId == 3}">
                                                        <c:if test="${not empty item.customerOrder.invoice && item.customerOrder.invoice.invoiceStatus == 'RELEASED'}">
                                                            |
                                                            <a href="${pageContext.request.contextPath}/invoice?orderId=${item.customerOrder.customerOrderId}"
                                                               style="color: #0284c7; font-weight: bold; text-decoration: none;">Xem Hóa đơn</a>
                                                        </c:if>
                                                    </c:when>
                                                    <c:otherwise>
                                                        |
                ...
                <div>
                    <c:if test="${totalPages > 1}">
                        <c:set var="queryParams"
                               value="search=${action}&keyword=${keyword}&sortBy=${sortBy}&sortOrder=${sortOrder}" />
