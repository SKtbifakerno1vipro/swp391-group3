                ...
                <div class="dashboard-shell">
                    ...
                    <main class="main legacy-page">
                        ...
                        <c:if test="${sessionScope.user.roleId != 3}">
                            <p><a href="${pageContext.request.contextPath}/quotation-create">Create New Quotation</a>
                            </p>
                        </c:if>
                                        ...
                                        <c:forEach items="${quotationList}" var="quotation">
                                            <tr>
                                                ...
                                                <td>
                                                    <c:choose>
                                                        <c:when
                                                            test="${quotation.hasContract && quotation.contractId != null}">
                                                            <a href="quotation-detail?id=${quotation.quotationId}">View
                                                                Detail</a>
                                                        </c:when>
                                                    ...
                                                    <%-- Chi hien nut tao hop dong neu trang thai la ACCEPTED. --%>
                                                        <c:if test="${quotation.quotationStatus == 'ACCEPTED'}">
                                                            <c:choose>
                                                                <c:when
                                                                    test="${quotation.hasContract && quotation.contractId != null}">
                                                                    | <a href="contract-detail?id=${quotation.contractId}"
                                                                        style="color: blue; font-weight: bold;">View
                                                                        Contract</a>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <c:if test="${sessionScope.user.roleId != 3}">
                                                                        | <a href="contract-save?quotationId=${quotation.quotationId}"
                                                                            style="color: green; font-weight: bold;">Create
                                                                            Contract</a>
                                                                    </c:if>
                                                                </c:otherwise>
                                                            </c:choose>
                        ...
                        <%-- Thanh phan trang giong Role module --%>
                            <div class="pagination" style="margin-top: 20px;">
                                <c:if test="${currentPage > 1}">
                                    <a class="page-link"
                                        href="${pageContext.request.contextPath}/quotation-list?page=${currentPage - 1}&search=${searchText}&status=${status}&fromDate=${fromDate}&toDate=${toDate}">&lt;</a>
                                </c:if>
                                <c:forEach begin="1" end="${totalPages}" var="i">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <a class="page-link"
                                                href="${pageContext.request.contextPath}/quotation-list?page=${i}&search=${searchText}&status=${status}&fromDate=${fromDate}&toDate=${toDate}">${i}</a>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
