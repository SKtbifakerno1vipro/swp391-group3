...
<html lang="en">
    <head>
        ...
        <style>
            :root { --bg:#faf6f0; --surface:#fffaf3; --surface-soft:#f0ece4; --surface-strong:#e4e0d8; --text:#2e3230; --muted:#646b66; --primary:#4a7c59; --primary-soft:#dcefe1; --secondary:#7a6148; --tertiary:#b1812f; --danger:#b83230; --danger-soft:#ffdad8; --line:#ddd5c9; --shadow:0 18px 45px rgba(46,50,48,.08); }
            * { box-sizing:border-box; }
            ...
            .button { display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:11px 16px; border:1px solid var(--line); border-radius:999px; background:var(--surface); color:var(--text); font-weight:800; cursor:pointer; font-family:inherit; }
            ...
            .field input { width:100%; padding:13px 14px; border:1px solid var(--line); border-radius:16px; background:#fff; color:var(--text); font:inherit; font-weight:700; outline:none; }
            .field input:read-only { background:var(--surface-soft); color:var(--muted); }
            .permission-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(230px,1fr)); gap:12px; }
            .permission-card { display:flex; align-items:center; gap:12px; padding:14px; border:1px solid var(--line); border-radius:18px; background:#fffdf8; font-weight:800; }
            .permission-card .material-symbols-outlined { color:var(--primary); }
            .terra-checkbox { appearance:none; width:22px; height:22px; border:2px solid #c4c8bc; border-radius:7px; background:#fff; display:inline-grid; place-items:center; cursor:pointer; transition:.2s ease; }
            .terra-checkbox:checked { background:var(--primary); border-color:var(--primary); }
            .terra-checkbox:checked::after { content:'check'; font-family:'Material Symbols Outlined'; color:#fff; font-size:17px; line-height:1; }
            table { width:100%; border-collapse:collapse; min-width:620px; }
            th { padding:15px 18px; color:var(--muted); text-align:left; font-size:12px; letter-spacing:.05em; text-transform:uppercase; background:#fbf7f0; }
            td { padding:16px 18px; border-top:1px solid var(--line); font-weight:700; }
            .table-wrap { overflow-x:auto; border-radius:20px; border:1px solid var(--line); }
            .alert { padding:14px 16px; border-radius:16px; margin-bottom:18px; background:var(--danger-soft); color:var(--danger); font-weight:800; }
            .empty-state { padding:24px; border-radius:18px; background:var(--surface-soft); color:var(--muted); text-align:center; font-weight:800; }
            @media (max-width:1080px) { .role-shell { grid-template-columns:1fr; } .sidebar { position:static; height:auto; } .nav-group { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); } .info-grid,.form-grid { grid-template-columns:1fr; } }
            @media (max-width:720px) { .main { padding:22px 16px; } .page-top,.panel-head { align-items:flex-start; flex-direction:column; } .actions { width:100%; justify-content:flex-start; } }
        </style>
        ...
        <div class="role-shell">
            ...
            <main class="main">
                <section class="page-top">
                    ...
                    <div class="actions"><a class="button" href="${pageContext.request.contextPath}/role-list"><span class="material-symbols-outlined">arrow_back</span>Back to roles</a><a class="button primary" href="${pageContext.request.contextPath}/edit-role-permissions?roleId=${role.roleId}"><span class="material-symbols-outlined">tune</span>Edit permissions</a></div>
                </section>
                <section class="panel">
