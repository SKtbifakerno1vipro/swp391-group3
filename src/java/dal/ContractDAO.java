...
public class ContractDAO extends DBContext {
    ...
    public boolean update(Contract c) {
        String sql = "UPDATE customer_contract SET contract_content = ?, contract_status = ?, updated_by = ?, updated_at = GETDATE() WHERE customer_contract_id = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, c.getContractContent());
            ps.setString(2, c.getContractStatus());
            ps.setInt(3, c.getUpdatedBy());
            ps.setInt(4, c.getContractId());
