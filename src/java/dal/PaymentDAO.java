...
public class PaymentDAO extends DBContext {
    ...
    public List<Payment> getCompletedPaymentsSince(Timestamp sinceTime, Integer customerUserId) {
        List<Payment> list = new ArrayList<>();
        StringBuilder sql = new StringBuilder(
                "SELECT p.*, c.contract_number, u.full_name as customer_name "
                + "FROM payment p "
                ...
                + "JOIN customer cust ON c.customer_id = cust.customer_id "
                + "JOIN [user] u ON cust.user_id = u.user_id "
                + "WHERE p.payment_status = 'COMPLETED' AND p.paid_at > ? "
        );
        if (customerUserId != null) {
            sql.append("AND p.created_by = ? ");
        }
        sql.append("ORDER BY p.paid_at ASC");
